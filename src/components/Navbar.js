...
export default function Navbar() {
  ...
  const isStudio =
    ...
    pathname === "/model-secim" ||
    pathname.startsWith("/model-secim/");
  ...
  // ✅ STÜDYO MODU: MARQUEE + FULL NAVBAR YOK
  // ✅ SADECE: GERİ DÖN + SEPET (sepet drawer çalışsın diye sepetAcik burada da var)
  if (isStudio) {
      // ✅ TASARIM (STÜDYO) sayfasında navbar + marquee görünmesin
  if (pathname?.startsWith("/tasarim")) {
    return null;
  }
    ...
    return (
      <>
        {/* STÜDYO MINI BAR */}
        <div className="fixed top-0 left-0 w-full z-[120] h-[64px] bg-black/80 backdrop-blur-md border-b border-zinc-800 flex items-center justify-between px-4 md:px-6">
          <button
            onClick={() => router.back()}
            className="text-zinc-300 hover:text-white transition uppercase text-xs font-bold tracking-widest"
          >
            ← Geri Dön
          </button>
          ...
          <button
            ...
            className="text-white hover:text-zinc-300 transition hover:bg-zinc-800 p-2 rounded-full relative"
            aria-label="Sepet"
          >
            <ShoppingBag size={20} strokeWidth={2} />
            {cart.length > 0 && (
              <span className="absolute top-0 right-0 w-4 h-4 bg-red-600 text-white text-[9px] flex items-center justify-center rounded-full font-bold">
                {cart.length}
              </span>
            )}
          </button>
        ...
        {/* SEPET DRAWER (Aynı şekilde dursun) */}
        {sepetAcik && (
            ...
            <div className="absolute top-0 right-0 h-full w-[90%] md:w-[450px] bg-zinc-950 border-l border-zinc-800 shadow-2xl animate-in slide-in-from-right-10 duration-300 flex flex-col">
                    ...
                    <div key={`${item.id}-${index}`} className="flex gap-4">
                      <div className="w-20 h-24 bg-zinc-900 rounded-lg overflow-hidden flex-shrink-0 border border-zinc-800">
                        {item.image ? (
                          ...
                          <div className="w-full h-full bg-zinc-800 flex items-center justify-center text-zinc-500 text-[9px]">Görsel Yok</div>
                        )}
                      </div>
                          ...
                          <div className="flex flex-col mt-1 gap-1">
                            ...
                            {item.color && (
                              <div className="flex items-center gap-2 text-xs text-zinc-500">
                                Renk:
                                <div className="w-3 h-3 rounded-full border border-zinc-600" style={{ backgroundColor: item.color }} />
                              </div>
                            )}
                          </div>
                        </div>
              ...
              {cart.length > 0 && (
                <div className="p-6 border-t border-zinc-900 bg-zinc-950">
                  ...
                  <button onClick={() => router.push('/odeme')} className="w-full bg-white text-black py-4 rounded-full font-black uppercase tracking-[0.2em] hover:bg-zinc-200 transition flex items-center justify-center gap-2">
